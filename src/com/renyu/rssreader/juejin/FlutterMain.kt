...
private fun checkExists(title: String) : Boolean {
    ...
    val value = okHttpHelper.okHttpUtils.syncGet(Params.baseUrl + "classes/Flutter?where="+jsonObject.toString(), Params.head()).body().string()
    var isExists=false
    if (value==null) {
        isExists=true
    }
...
private fun update(title: String, avatarLarge: String?, username: String, originalUrl: String, screenshot: String?, summaryInfo: String) {
    ...
    okHttpHelper.okHttpUtils.syncPostJson(Params.baseUrl + "classes/Flutter", jsonObject.toString(), Params.head())
}